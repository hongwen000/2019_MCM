...
bool flag = true;
auto get_ret_when_T(int T, const std::vector<int> sinks, const std::vector<int> sources, int require, double alpha)
{
...
{
    double alpha =  stoi(string(argv[1]));
    ...
    {
        T += 1;
        auto ret = get_ret_when_T(T, sinks, sources, require, alpha);
        flow = ret.second;
        best_ret = ret.first;
        if(T > 10000) exit(0);
    }
    cout <<  " "<< alpha << " " << best_ret - 1640757<< endl;
    return 0;
}
